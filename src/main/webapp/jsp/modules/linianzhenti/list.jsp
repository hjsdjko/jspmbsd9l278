...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">历年真题管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>历年真题列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var liniantimuCell  = document.createElement('td');

						  var liniantimuFile = document.createElement('button');
			  liniantimuFile.setAttribute('class','btn-table')
			  liniantimuFile.innerHTML = '下载';
			  var liniantimuFileAttr = "download('" +baseUrl+ item.liniantimu + "')"
			  liniantimuFile.setAttribute('onclick',liniantimuFileAttr);
        	          liniantimuCell.appendChild(liniantimuFile);


			row.appendChild(liniantimuCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("linianzhenti/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discusslinianzhenti/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"linianzhenti");
